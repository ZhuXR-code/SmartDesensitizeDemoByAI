    ...
    <el-row :gutter="20" class="kpi-row">
      <el-col :xs="24" :sm="12" :md="8" :lg="4" v-for="(kpi, index) in kpis" :key="index">
        <el-card shadow="hover" class="kpi-card">
          <div class="kpi-icon" style="background: rgba(192,137,221,0.25);">
            <el-icon size="24" style="color: #420D4B;"><component :is="kpi.icon" /></el-icon>
          </div>
        ...
        <el-card shadow="hover" class="insight-card">
          ...
          <div class="insight-list">
            <div v-for="(insight, idx) in insights" :key="idx" class="insight-item">
              <div class="insight-icon" style="background: rgba(192,137,221,0.25);">
                <el-icon size="18" style="color: #420D4B;"><component :is="insight.icon" /></el-icon>
              </div>
...
const initSecurityChart = () => {
  ...
  chart.setOption({
    ...
    legend: { orient: 'vertical', right: 10, top: 'center', textStyle: { fontSize: 11, color: '#797382' } }, // 深灰紫色，清晰可见
    series: [{
      type: 'pie',
      radius: ['40%', '65%'],
      center: ['35%', '50%'],
...
.report-title {
  font-size: 24px;
  font-weight: 600;
  color: #FFFFFF; /* 白色，在深色背景上清晰醒目 */
  margin: 0 0 6px 0;
}

.report-subtitle {
  font-size: 13px;
  color: #E1DEE9; /* 极浅紫灰色，在深色背景上柔和清晰 */
  margin: 0;
}
...
.kpi-value {
  font-size: 22px;
  font-weight: bold;
  color: #303133; /* 深灰色，突出显示 */
  line-height: 1.2;
}

.kpi-label {
  font-size: 12px;
  color: #797382; /* 深灰紫色，提高对比度 */
  margin-top: 4px;
}
...
.value-number {
  font-size: 36px;
  font-weight: bold;
  background: linear-gradient(135deg, #7796CB 0%, #576490 100%); /* 蓝色系渐变 */
...
.value-title {
  font-size: 15px;
  font-weight: 600;
  color: #303133; /* 深灰色，标题醒目 */
  margin: 10px 0 8px;
}

.value-desc {
  font-size: 12px;
  color: #797382; /* 深灰紫色，清晰易读 */
  line-height: 1.6;
}
...
.tech-badge {
  ...
  background: linear-gradient(135deg, rgba(244, 253, 175, 0.4) 0%, rgba(239, 221, 141, 0.3) 100%);
  color: #210124;
  font-size: 11px;
  font-weight: bold;
  padding: 2px 10px;
...
.metric-num {
  font-size: 28px;
  font-weight: bold;
  color: #303133; /* 深灰色，指标数字突出 */
}

.metric-unit {
  font-size: 12px;
  color: #797382; /* 深灰紫色，提高可见度 */
  margin-left: 4px;
}

.tech-desc {
  font-size: 12px;
  color: #797382; /* 深灰紫色，清晰可读 */
  line-height: 1.6;
  margin-bottom: 10px;
}
...
.desc-detail {
  font-size: 12px;
  color: #797382; /* 深灰紫色，提高可读性 */
  margin-left: 8px;
}

/* 修复el-descriptions第一列白色背景问题 */
.compliance-card :deep(.el-descriptions__label) {
  background: rgba(33, 1, 36, 0.4) !important;
  color: #303133 !important; /* 深灰色，标签清晰 */
  font-weight: 500 !important;
}

.compliance-card :deep(.el-descriptions__content) {
  background: rgba(33, 1, 36, 0.3) !important;
  color: #797382 !important; /* 深灰紫色，清晰可读 */
}

.compliance-card :deep(.el-descriptions__cell) {
  border-color: rgba(244, 253, 175, 0.1) !important;
}
