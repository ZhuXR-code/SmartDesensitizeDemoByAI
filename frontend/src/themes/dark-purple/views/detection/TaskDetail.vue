        ...
        <el-row :gutter="20">
          <el-col :span="4" v-for="(count, lang) in task.language_distribution" :key="lang">
            <el-statistic :title="languageMap[lang] || lang" :value="count">
              <template #suffix>
                <span style="font-size: 12px; color: rgba(239, 221, 141, 0.5);">条</span>
              </template>
            </el-statistic>
          </el-col>
        </el-row>
          ...
          <el-table-column prop="confidence" label="置信度" width="150" align="center">
            <template #header>
              ...
              >
                ...
                <el-icon style="margin-left: 4px; cursor: help; color: rgba(239, 221, 141, 0.5);"><QuestionFilled /></el-icon>
              </el-tooltip>
            </template>
            <template #default="{ row }">
              <el-progress 
...
const getConfidenceColor = (confidence) => {
  if (confidence >= 0.9) return '#F4FDAF'
  if (confidence >= 0.7) return '#EFDD8D'
