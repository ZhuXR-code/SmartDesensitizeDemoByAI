      ...
      <div v-if="currentRule">
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item label="配置信息">
            <div style="background: rgba(33, 1, 36, 0.4); padding: 12px; border-radius: 4px; font-family: monospace; font-size: 12px;">
              ...
              <div style="margin-top: 8px; color: rgba(239, 221, 141, 0.5); font-size: 11px;">
        ...
        <el-table :data="[currentRule.example || {}]" size="small" border>
          <el-table-column label="脱敏前" prop="before">
            <template #default="{ row }">
              <span style="color: #FCA5A5; font-weight: bold;">{{ row.before || '暂无示例' }}</span>
            </template>
          </el-table-column>
          <el-table-column label="脱敏后" prop="after">
            <template #default="{ row }">
              <span style="color: #F4FDAF; font-weight: bold;">{{ row.after || '暂无示例' }}</span>
            </template>
          </el-table-column>
        </el-table>
