<template>
  <div class="platform-report">
    ...
    <!-- 核心指标总览 -->
    <div class="kpi-grid">
      <div v-for="(kpi, index) in kpis" :key="index" class="kpi-grid-item">
        <el-card shadow="hover" class="kpi-card">
          <div class="kpi-icon" style="background: #f0f0f0;">
            <el-icon size="24" style="color: #666;"><component :is="kpi.icon" /></el-icon>
          </div>
        ...
        <el-card shadow="hover" class="insight-card">
          ...
          <div class="insight-list">
            <div v-for="(insight, idx) in insights" :key="idx" class="insight-item">
              <div class="insight-icon" style="background: #f0f0f0;">
                <el-icon size="18" style="color: #666;"><component :is="insight.icon" /></el-icon>
              </div>
...
const kpis = computed(() => [
  ...
  {
    icon: 'Cpu',
    value: overview.value.total_ai_detection_tasks || 0,
    label: 'AI识别任务',
    trend: `本月+${overview.value.monthly_ai_detection_tasks || 0}`,
    trendType: 'primary'
  },
  {
    icon: 'Lock',
  ...
  {
    icon: 'Check',
    value: `${overview.value.ai_accuracy_rate || overview.value.accuracy_rate || 0}%`,
    label: 'AI识别准确率',
    trend: 'AI驱动',
    trendType: 'danger'
  },
...
const valueSummaries = computed(() => [
  {
    number: `${technology.value.ai_model?.accuracy_rate || 95}%`,
    title: 'AI识别准确率',
    description: '基于LLM大模型的语义级敏感判断，支持多模型（OpenAI/DeepSeek/Qwen等）灵活切换，准确率95%+。'
  },
  {
  ...
  },
  {
    ...
    title: '关联造数应用',
    description: '独创的基于密钥的确定性脱敏算法，保证跨表数据一致性，30组独立密钥实现安全隔离。'
  }
])
